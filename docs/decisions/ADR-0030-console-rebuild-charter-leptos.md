...
| The shared contracts crate exists and `backend/openapi/openapi.yaml` is generated from it | **OPEN** | At acceptance, `utoipa` appeared nowhere in `backend/Cargo.toml` or `backend/Cargo.lock`; the 35,935-line OpenAPI document was hand-maintained, served verbatim by `include_str!`, and still used the pre-rename title. As of 2026-08-03 the title is `Console API`, but no contracts-crate emitter or schema-to-wire fidelity proof exists, so the gate remains OPEN. |
