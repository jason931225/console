...
Start from a new clone and from no local branch other than `main`:

```sh
git clone git@github.com:jason931225/console.git
cd console
git switch main
git pull --ff-only
...
The consolidation acceptance condition is that PRs #552 and #562 both report
`MERGED`, the open-PR query is empty, the latest `main` CI and authority-binding
run are green, and release `v0.3.1` exists. If any condition is false, treat that
as an interrupted merge operation—not permission to recover an old local branch.
...
The following useful final nets were carried into PR #562. Source identities are
recorded so future archaeology can distinguish reviewed work from coincidental
similarity; none is required after the disk is wiped.
...
| CI/tooling ratchets | Exact executed-test named sets, Cargo/feature reachability, credential argv hardening, local-CI parity, and removal of dead post-pivot tooling only | reduced from `09f147a7`, `26491630`, and `eb60e2e4`; see PR #562 diff/review |
...
| Cosskorea domain code (`f3a0d9d8` and the eight dirty root files) | Rejected after five correction rounds. Six sustained defects include session-takeover risk, unbound WebAuthn evidence, unsafe consent provenance, unusable ordinary-user recovery, missing served-certificate proof, and broken rollback cleanup. The blocker ledger is the only durable artifact. |
...
contained no durable unique plan. Exactly three planning inputs were retained,
byte-for-byte:
...
that the approved plan was subsequently implemented and verified. A fresh
session should continue from current program state; it should not rerun the old
Ultragoal as if implementation were pending.
...
areas, and the remaining OMX/OMC runtime directories.
