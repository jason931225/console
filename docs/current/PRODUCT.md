...
5. A Leptos SSR frontend. ADR-0030 substrate gates are green and `Layer::Ui` is accepted (ADR-0041); first full-depth vertical is payroll execution.
...
- Frontend work is **resumed** (2026-08-25, owner decision; `Layer::Ui` accepted by ADR-0041). First full-depth vertical is payroll execution. Stack: Leptos 0.9.0-beta (crates.io) with SSR, islands, and selective WASM hydration; 0.8.x is the rollback if beta blocks production. The client has no business authority; deny-by-omission is server composition. Import/export is not the data-entry base except 자료실 (later, not this slice). The comms rail is not in this slice. A screen still does not count as shipped without a stable contracts-backed SSR shell and real E2E evidence (ADR-0025 §4).
