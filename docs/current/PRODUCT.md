...
- Frontend **authorization** for the payroll vertical is resumed (2026-08-25, owner decision; `Layer::Ui` accepted by ADR-0041). That does not clear the remaining frontend **HOLD**. First full-depth vertical is payroll execution. Stack: Leptos 0.9.0-beta (crates.io) with SSR, islands, and selective WASM hydration; 0.8.x is the rollback if beta blocks production. The client has no business authority; deny-by-omission is server composition. Import/export is not the data-entry base except 자료실 (later, not this slice). The comms rail is not in this slice. Remaining **HOLD**: a mounted contracts-backed SSR shell and real E2E evidence before a screen counts as shipped (ADR-0025 §4). Absence of a mounted shell stays green until the change that mounts one.
