...
type Props = {
  ...
  capabilities: { canRead: boolean; canManage: boolean; canReadCost: boolean };
};
...
export function AssetWorkspace({ api, capabilities }: Props) {
  ...
  const [substitution, setSubstitution] = useState<SubstituteAssignment>();
  ...
  const load = useCallback(async (q = query) => {
    ...
    try {
      const response = await api.GET("/api/v1/equipment/list", {
        params: { query: { q: q || undefined, limit: 50, offset: 0 } },
      });
      if (!response.data) throw new Error("장비 목록 응답이 없습니다.");
  ...
  async function open(row: EquipmentListItem) {
    ...
    try {
      const [timeline, versions, candidates, transfers, cost] = await Promise.all([
        api.GET("/api/v1/equipment/{id}/timeline-graph", { params: { path: { id: row.equipment_id } } }),
        api.GET("/api/v1/equipment/{id}/versions", { params: { path: { id: row.equipment_id } } }),
        capabilities.canManage
          ? api.GET("/api/v1/equipment/{id}/substitutes", { params: { path: { id: row.equipment_id } } })
          : Promise.resolve({ data: undefined }),
        capabilities.canManage
          ? api.GET("/api/v1/equipment/{id}/ownership-transfer-requests", { params: { path: { id: row.equipment_id } } })
          : Promise.resolve({ data: undefined }),
        capabilities.canReadCost
          ? api.GET("/api/v1/financial/equipment/{equipmentId}/lifecycle-cost", { params: { path: { equipmentId: row.equipment_id } } })
          : Promise.resolve({ data: undefined }),
      ]);
      setSelected({
        row,
  ...
  async function register(event: SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.data) throw new Error("장비 등록 응답이 없습니다.");
      form.reset();
  ...
  async function rollback(version: number) {
    ...
    try {
      ...
      if (!response.data) throw new Error("되돌림 응답이 없습니다.");
      setNotice(`버전 ${String(response.data.version)}으로 되돌림 이력을 추가했습니다.`);
  ...
  async function assign(candidate: SubstituteCandidate) {
    ...
    try {
      ...
      if (!response.data) throw new Error("대차 배정 응답이 없습니다.");
  ...
  async function returnSubstitution() {
    ...
    setBusy(true); setError(undefined);
    try {
      const response = await api.POST("/api/v1/equipment-substitutions/{id}/return", { params: { path: { id: substitution.id } }, body: {} });
      if (!response.data) throw new Error("대차 반납 응답이 없습니다.");
  ...
  async function createTransfer(event: SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.data) throw new Error("소유권 이전 요청 응답이 없습니다.");
  ...
  async function decideTransfer(id: string, decision: "approve" | "reject") {
    ...
    try {
      ...
      if (!response.data) throw new Error("결정 응답이 없습니다.");
    ...
    <div className="mx-auto grid max-w-[1600px] gap-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        {capabilities.canManage ? <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" size="sm" variant="secondary" onClick={() => { setShowImport((open) => !open); }}>엑셀 가져오기</Button>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(380px,0.9fr)]">
        ...
        {selected ? <AssetDetail detail={selected} capabilities={capabilities} busy={busy} substitution={substitution} onRollback={rollback} onAssign={assign} onReturn={returnSubstitution} onTransfer={createTransfer} onDecide={decideTransfer} /> : <Card><p role="status" className="text-sm text-steel">목록에서 장비를 선택하면 이력, 관계 및 비용을 확인합니다.</p></Card>}
      </section>
    </div>
  </main>;
}
...
function AssetDetail({ detail, capabilities, busy, substitution, onRollback, onAssign, onReturn, onTransfer, onDecide }: { detail: Detail; capabilities: Props["capabilities"]; busy: boolean; substitution?: SubstituteAssignment; onRollback: (version: number) => Promise<void>; onAssign: (candidate: SubstituteCandidate) => Promise<void>; onReturn: () => Promise<void>; onTransfer: (event: SyntheticEvent<HTMLFormElement>) => Promise<void>; onDecide: (id: string, decision: "approve" | "reject") => Promise<void> }) {
