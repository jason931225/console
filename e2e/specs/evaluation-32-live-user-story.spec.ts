 ...
 *
 *   MNT_DEV_AUTH_E2E=1 VITE_CONSOLE_DEV_PREVIEW=1 \
 *     node scripts/dev-up.mjs bootstrap
 *   MNT_DEV_AUTH_E2E=1 npx playwright test \
 *     --project=dev-auth e2e/specs/evaluation-32-live-user-story.spec.ts
 *
...
const cycleName = `E2E 평가 ${runId.slice(0, 8)}`;
const subjectEmployeeId = randomUUID();
const subjectEmployeeName = `E2E 평가 대상 ${runId.slice(0, 8)}`;
const isolatedOrgId = randomUUID();
const isolatedOrgSlug = `e2e-evaluation-${runId.replaceAll("-", "").slice(0, 16)}`;
...
function assertDevOnlyEnvironment(): void {
  if (process.env.MNT_DEV_AUTH_E2E !== "1") {
    throw new Error("EVALUATION-32 may run only with MNT_DEV_AUTH_E2E=1.");
  }
}
...
async function loginAs(page: Page, roleLabel: string, orgId = ORG_ID): Promise<void> {
  ...
  await switcher.getByRole("combobox", { name: "역할" }).selectOption({ label: roleLabel });
  if (orgId !== ORG_ID) {
...
function provisionPrerequisites(): void {
  ...
  const users = execSql(`
    ...
    COMMIT;
  `).split("\n");
  const ids = new Map(users.filter(Boolean).map((row) => row.split("|") as [string, string]));
  adminUserId = ids.get(adminPhone) ?? "";
  executiveUserId = ids.get(executivePhone) ?? "";
  if (!adminUserId || !executiveUserId) {
    throw new Error("dev-auth personas were not provisioned through the login UI");
  }
  ...
  execSql(`
    ...
    INSERT INTO employees (
      ...
      ${sqlLiteral(subjectEmployeeId)}::uuid, ${sqlLiteral(ORG_ID)}::uuid,
      'E2E', ${sqlLiteral(subjectEmployeeName)}, 'evaluation-live-e2e.xlsx',
      'evaluation', 1, ${sqlLiteral(`evaluation-live-e2e-${runId}`)},
      '2024-01-02', 'ACTIVE', 'E2E 평가', false
    );
    UPDATE users
    SET employee_id = ${sqlLiteral(subjectEmployeeId)}::uuid
  ...
  execOwnerSql(`
    ...
    VALUES (
      ${sqlLiteral(isolatedOrgId)}::uuid,
      ${sqlLiteral(isolatedOrgSlug)},
      ${sqlLiteral(`E2E 평가 격리 ${runId.slice(0, 8)}`)},
      'ACTIVE'
    );
  `);
}

function provisionIsolatedTenant(): void {
  execOwnerSql(`
    INSERT INTO organizations (id, slug, name, status)
    VALUES (
      ${sqlLiteral(isolatedOrgId)}::uuid,
...
async function openEvaluation(page: Page): Promise<void> {
  ...
  await expect(page.getByRole("heading", { name: "평가", level: 1 })).toBeVisible({
    timeout: 15_000,
  });
}

async function createCycleAndSubject(page: Page, refreshPreflight = false): Promise<void> {
  await openEvaluation(page);
  await page.getByLabel("사이클 이름").fill(cycleName);
  await page.getByLabel("기간").fill("2026-E2E");
  await page.getByLabel("마감일").fill("2026-12-31");
  await page.getByRole("button", { name: "생성", exact: true }).click();
  await expect(page.getByRole("heading", { name: cycleName, level: 2 })).toBeVisible();

  await page.getByLabel("직원 검색").fill(subjectEmployeeName);
  const result = page.getByRole("list", { name: "직원 검색 결과" });
  await expect(result.getByRole("button", { name: new RegExp(subjectEmployeeName) })).toBeVisible({
    timeout: 15_000,
  });
  await result.getByRole("button", { name: new RegExp(subjectEmployeeName) }).click();
  await page.getByLabel("평가자").selectOption(adminUserId);
  await page.getByRole("button", { name: "추가", exact: true }).click();
  const subjectRow = page
    .getByRole("list", { name: "대상자" })
    .getByRole("button", { name: new RegExp(subjectEmployeeName) });
  ...
  const goalsSaved = page.waitForResponse(
    (response) =>
      ...
      /\/api\/v1\/evaluation\/subjects\/[^/]+\/goals$/.test(new URL(response.url()).pathname) &&
      response.status() === 200,
  );
...
async function bootstrapOpenCycle(page: Page): Promise<void> {
  await loginAs(page, "관리자");
  await loginAs(page, "임원");
  await loginAs(page, "최고 관리자");
  provisionPrerequisites();
  await createCycleAndSubject(page, true);
}

async function submitReview(page: Page, kind: "자기평가" | "관리자 평가"): Promise<void> {
  const task = page.getByRole("list", { name: "내 평가 할 일" }).getByRole("listitem").filter({ hasText: kind });
  ...
  if (kind === "관리자 평가") {
    await dialog.getByLabel("개체 코드").fill(`E2E-EVIDENCE-${runId.slice(0, 8)}`);
    await dialog.getByLabel("설명").fill("현장 검증 기록");
    await dialog.getByRole("button", { name: "증빙 추가" }).click();
  }
  await dialog.getByRole("group", { name: "등급" }).getByRole("button", { name: "A" }).click();
  await dialog.getByLabel("평가 의견 (선택)").fill(`${kind} ${runId.slice(0, 8)}`);
...
test.describe("EVALUATION-32 real dev-auth story", () => {
  ...
  test("administrator UI persists an enrolled cycle through manager review, calibration, finalization, and ledger readback", async ({ page }) => {
    ...
    await createCycleAndSubject(page);
    ...
    await loginAs(page, "관리자");
    await openEvaluation(page);
    await submitReview(page, "자기평가");
    await submitReview(page, "관리자 평가");

    await loginAs(page, "최고 관리자");
    await openEvaluation(page);
    await page.getByRole("button", { name: new RegExp(cycleName) }).click();
    await page.getByRole("button", { name: "조정 시작", exact: true }).click();
    await page.getByRole("button", { name: new RegExp(subjectEmployeeName) }).click();
    await page.getByRole("group", { name: "등급" }).getByRole("button", { name: "A" }).click();
    ...
    await page.getByRole("button", { name: subjectEmployeeName, exact: true }).last().click();
    await expect(page.getByRole("heading", { name: subjectEmployeeName, level: 2 })).toBeVisible();
    await expect(page.getByRole("list", { name: "평가 이력" }).getByText(cycleName)).toBeVisible({
      timeout: 15_000,
    });
  });

  test("RED: linked employee sees and submits their own SELF review before manager review", async ({ page }) => {
  ...
  test("member role is denied and an isolated tenant cannot read the source tenant cycle", async ({ page }) => {
    ...
    await loginAs(page, "최고 관리자", isolatedOrgId);
    await openEvaluation(page);
    await expect(page.getByText(cycleName, { exact: true })).toHaveCount(0);
    await expect(page.getByText("평가 사이클이 없습니다. 새 사이클을 생성하세요.")).toBeVisible();
  });
});
