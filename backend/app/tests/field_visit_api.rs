...
async fn authenticated_runtime_role_completes_field_visit_story(pool: PgPool) {
    ...
    let (status, ticket) = send(&rt, &keys, "POST", TICKETS, &token, Some(json!({
    ...
    assert!(ticket["site_id"].is_null(), "intake starts unlinked: {ticket}");
    let ticket_id = ticket["id"].as_str().unwrap().to_owned();

    // Link the site: customer_id denormalizes from the site, names resolve.
    let (status, linked) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/link"), &token,
        Some(json!({"site_id": site})), None).await;
    ...
    assert_eq!(linked["customer_id"].as_str().unwrap(), customer.to_string());
    assert_eq!(linked["customer_name"], "화성정밀");

    // An empty link body is a validation error, not a silent no-op.
    let (status, empty) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/link"), &token,
        Some(json!({})), None).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY, "empty link: {empty}");

    // Link the dispatched work order (site matches — the guardrail passes).
    let (status, linked) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/link"), &token,
        Some(json!({"work_order_id": work_order})), None).await;
    assert_eq!(status, StatusCode::OK, "link work order: {linked}");
    assert_eq!(linked["work_order_id"].as_str().unwrap(), work_order.to_string());
    ...
    let (status, breached) = send(&rt, &keys, "GET", &format!("{FIELD_SITES}?sla=BREACHED"), &token, None, None).await;
    ...
    let early = json!({"kind": "CUSTOMER_ACCEPTED", "channel": "IN_PERSON", "accepted_by": "김담당"});
    let (status, body) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/acceptance"), &token,
        Some(early.clone()), Some("field-acceptance-key-0001")).await;
    assert_eq!(status, StatusCode::CONFLICT, "acceptance requires RESOLVED: {body}");

    for to in ["IN_PROGRESS", "RESOLVED"] {
        let (status, body) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/transition"), &token,
            Some(json!({"to_status": to})), None).await;
        assert_eq!(status, StatusCode::OK, "transition to {to}: {body}");
    }

    // Accept: append-only evidence + the existing RESOLVED→CLOSED edge.
    let (status, acceptance) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/acceptance"), &token,
        Some(early.clone()), Some("field-acceptance-key-0001")).await;
    assert_eq!(status, StatusCode::CREATED, "record acceptance: {acceptance}");
    ...
    let (status, closed) = send(&rt, &keys, "GET", &format!("{TICKETS}/{ticket_id}"), &token, None, None).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(closed["ticket"]["status"], "CLOSED", "accept closes: {closed}");

    // Idempotency: same key + same request replays the stored acceptance;
    // same key + different request is a conflict.
    let (status, replay) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/acceptance"), &token,
        Some(early), Some("field-acceptance-key-0001")).await;
    assert_eq!(status, StatusCode::CREATED, "idempotent replay: {replay}");
    assert_eq!(replay["id"].as_str().unwrap(), acceptance_id);
    let (status, changed) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/acceptance"), &token,
        Some(json!({"kind": "CUSTOMER_ACCEPTED", "channel": "PHONE", "accepted_by": "김담당"})),
        Some("field-acceptance-key-0001")).await;
    ...
    let (_, ticket2) = send(&rt, &keys, "POST", TICKETS, &token, Some(json!({
    ...
    send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket2_id}/link"), &token, Some(json!({"site_id": site})), None).await;
    for to in ["IN_PROGRESS", "RESOLVED"] {
        send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket2_id}/transition"), &token, Some(json!({"to_status": to})), None).await;
    }
    let (status, noteless) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket2_id}/acceptance"), &token,
        Some(json!({"kind": "CUSTOMER_DECLINED", "channel": "PHONE", "accepted_by": "김담당"})),
        Some("field-acceptance-key-0002")).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY, "decline requires a note: {noteless}");
    ...
    let (_, reopened) = send(&rt, &keys, "GET", &format!("{TICKETS}/{ticket2_id}"), &token, None, None).await;
    assert_eq!(reopened["ticket"]["status"], "IN_PROGRESS", "decline reopens: {reopened}");
    let comments = reopened["comments"].as_array().unwrap();
    assert!(
        comments.iter().any(|c| c["body"] == "소음이 여전함" && c["is_internal_note"] == false),
        "decline note becomes a customer-visible comment: {reopened}"
    );

    // Detail: object + history layers all present and traversable.
    let (status, detail) = send(&rt, &keys, "GET", &format!("{FIELD_SITES}/{site}"), &token, None, None).await;
    ...
    assert_eq!(detail["sla"]["open"], 1, "reopened decline ticket stays open: {detail}");
    assert_eq!(detail["tickets"].as_array().unwrap().len(), 2);
    assert_eq!(detail["work_orders"].as_array().unwrap().len(), 1);
    assert_eq!(detail["work_orders"][0]["id"].as_str().unwrap(), work_order.to_string());
    ...
    let (status, queue) = send(&rt, &keys, "GET", &format!("{TICKETS}?site_id={site}"), &token, None, None).await;
    assert_eq!(status, StatusCode::OK);
    ...
    let acceptance_snaps: Vec<Option<String>> = sqlx::query_scalar("SELECT after_snap::text FROM audit_events WHERE action = 'support.ticket.acceptance'")
        .fetch_all(&pool).await.unwrap();
    assert_eq!(acceptance_snaps.len(), 2, "accept and decline are each audited once (replay is not re-audited)");
    for snap in acceptance_snaps.into_iter().flatten() {
        assert!(!snap.contains("김담당"), "accepted_by must never enter audit snapshots: {snap}");
    }
    let acceptance_rows: i64 = sqlx::query_scalar("SELECT count(*) FROM support_ticket_acceptances")
        .fetch_one(&pool).await.unwrap();
    assert_eq!(acceptance_rows, 2, "acceptance evidence is append-only, one row per verdict");
}
...
async fn field_console_denies_without_leakage(pool: PgPool) {
    ...
    assert_eq!(status, StatusCode::OK);
    assert_eq!(page["total"], 0, "out-of-scope site must not be counted: {page}");
    assert_eq!(page["items"].as_array().unwrap().len(), 0);
    let (status, hidden) = send(&rt, &keys, "GET", &format!("{FIELD_SITES}/{site_b}"), &token_a, None, None).await;
    assert_eq!(status, StatusCode::NOT_FOUND, "out-of-scope detail is 404: {hidden}");
    assert_eq!(hidden["error"]["code"], "not_found");
    assert!(!hidden.to_string().contains("다른지점"), "denial must not leak the site: {hidden}");

    // Linking an out-of-scope site from an in-scope ticket is also a 404.
    let (_, ticket) = send(&rt, &keys, "POST", TICKETS, &token_a, Some(json!({
        "branch_id": branch_a, "category": "OPERATIONAL", "priority": "LOW",
        "title": "권한 검증", "body": "범위 밖 현장 연결 시도"
    })), None).await;
    let ticket_id = ticket["id"].as_str().unwrap();
    let (status, denied) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/link"), &token_a,
        Some(json!({"site_id": site_b})), None).await;
    assert_eq!(status, StatusCode::NOT_FOUND, "out-of-scope site link is 404: {denied}");
    ...
    let (status, denied) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/link"), &member_token,
        Some(json!({"site_id": site_a})), None).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "link is AssigneeManage-gated: {denied}");
    assert_eq!(denied["error"]["code"], "forbidden");
    let (status, denied) = send(&rt, &keys, "POST", &format!("{TICKETS}/{ticket_id}/acceptance"), &member_token,
        Some(json!({"kind": "CUSTOMER_ACCEPTED", "channel": "IN_PERSON", "accepted_by": "김담당"})),
        Some("field-acceptance-key-0009")).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "acceptance is AssigneeManage-gated: {denied}");

    // A missing Idempotency-Key on acceptance is a validation error.
    let (status, missing) = send_without_idempotency(&rt, &keys, &format!("{TICKETS}/{ticket_id}/acceptance"), &token_a,
        json!({"kind": "CUSTOMER_ACCEPTED", "channel": "IN_PERSON", "accepted_by": "김담당"})).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY, "acceptance requires Idempotency-Key: {missing}");
}
...
async fn field_console_isolates_tenants_as_runtime_role(pool: PgPool) {
    ...
    sqlx::query("INSERT INTO organizations (id, slug, name) VALUES ($1, 'other-tenant', 'Other Tenant')")
        .bind(*other.as_uuid()).execute(&pool).await.unwrap();
    ...
    assert_eq!(page["total"], 0, "sibling tenant rows must not be counted: {page}");
    let (status, hidden) = send(&rt, &keys, "GET", &format!("{FIELD_SITES}/{site_knl}"), &token_other, None, None).await;
    assert_eq!(status, StatusCode::NOT_FOUND, "sibling tenant detail is 404: {hidden}");
    assert!(!hidden.to_string().contains("KNL 현장"), "denial must not leak the site: {hidden}");
}
...
impl Keys {
    ...
    fn token(&self, user: UserId, org: OrgId, role: &str, branches: Vec<BranchId>) -> String {
        JwtIssuer::from_es256_pem(JwtSettings { issuer: ISSUER.into(), audience: AUDIENCE.into(), access_token_ttl: Duration::minutes(15) }, self.private_pem.as_bytes(), self.public_pem.as_bytes()).unwrap()
            .issue_access_token(AccessTokenInput { subject: user, org_id: org, roles: vec![role.into()], branches, platform: false, view_as: false, read_only: false, display_name: None, feature_grants: Vec::new(), authz_subject_version: 0, authz_policy_version: 0, session_generation: 0, issued_at: OffsetDateTime::now_utc() }).unwrap()
    }
}
...
async fn send(pool: &PgPool, keys: &Keys, method: &str, uri: &str, token: &str, body: Option<Value>, idempotency_key: Option<&str>) -> (StatusCode, Value) {
    let request = Request::builder().method(method).uri(uri).header(header::AUTHORIZATION, format!("Bearer {token}")).header(header::CONTENT_TYPE, "application/json").header("Idempotency-Key", idempotency_key.unwrap_or("not-used-by-this-request")).body(body.map(|v| Body::from(serde_json::to_vec(&v).unwrap())).unwrap_or_else(Body::empty)).unwrap();
...
async fn send_without_idempotency(pool: &PgPool, keys: &Keys, uri: &str, token: &str, body: Value) -> (StatusCode, Value) {
    let request = Request::builder().method("POST").uri(uri).header(header::AUTHORIZATION, format!("Bearer {token}")).header(header::CONTENT_TYPE, "application/json").body(Body::from(serde_json::to_vec(&body).unwrap())).unwrap();
    dispatch(pool, keys, request).await
}

async fn dispatch(pool: &PgPool, keys: &Keys, request: Request<Body>) -> (StatusCode, Value) {
    let response = build_router(app_state(pool.clone(), keys.public_pem.clone()).unwrap()).oneshot(request).await.unwrap();
...
async fn seed_branch(pool: &PgPool, org: OrgId, name: &str) -> BranchId {
    let region: Uuid = sqlx::query_scalar("INSERT INTO regions (name, org_id) VALUES ($1, $2) RETURNING id").bind(format!("region-{name}")).bind(*org.as_uuid()).fetch_one(pool).await.unwrap();
    BranchId::from_uuid(sqlx::query_scalar("INSERT INTO branches (region_id, name, org_id) VALUES ($1, $2, $3) RETURNING id").bind(region).bind(name).bind(*org.as_uuid()).fetch_one(pool).await.unwrap())
}
...
async fn seed_user(pool: &PgPool, org: OrgId, role: &str, branch: BranchId) -> UserId {
    ...
    sqlx::query("INSERT INTO user_branches (user_id, branch_id, org_id) VALUES ($1, $2, $3)").bind(*user.as_uuid()).bind(*branch.as_uuid()).bind(*org.as_uuid()).execute(pool).await.unwrap();
    user
}

async fn seed_customer(pool: &PgPool, org: OrgId, branch: BranchId, name: &str) -> Uuid {
    sqlx::query_scalar("INSERT INTO registry_customers (branch_id, name, org_id) VALUES ($1, $2, $3) RETURNING id").bind(*branch.as_uuid()).bind(name).bind(*org.as_uuid()).fetch_one(pool).await.unwrap()
}

async fn seed_site(pool: &PgPool, org: OrgId, branch: BranchId, customer: Uuid, name: &str) -> Uuid {
...
async fn seed_equipment(pool: &PgPool, org: OrgId, branch: BranchId, customer: Uuid, site: Uuid) -> Uuid {
...
async fn seed_work_order(pool: &PgPool, org: OrgId, branch: BranchId, customer: Uuid, site: Uuid, equipment: Uuid, requested_by: UserId) -> Uuid {
...
async fn seed_attendance(pool: &PgPool, org: OrgId, branch: BranchId, user: UserId, work_order: Uuid, site: Uuid) {
