...
// lane-local build unblock (CAP-ORG-CONSOLE): `hash(&b)` requires Serialize.
...
async fn transition(
    ...
    let org = p.org_id;
    let actor = p.clone();
    // lane-local build unblock (CAP-ORG-CONSOLE): the borrowed status cannot
    // cross into the 'static audited closure.
    let to = to.to_owned();
